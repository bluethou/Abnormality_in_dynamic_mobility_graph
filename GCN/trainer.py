...
class Trainer():
	...
	def train(self):
		...
		for e in range(self.args.num_epochs):
			eval_train, nodes_embs, _, _  = self.run_epoch(self.splitter.train, e, 'TRAIN', grad = True)
			if len(self.splitter.dev)>0 and e>self.args.eval_after_epochs:
				eval_valid, temp_node_embs, _, _ = self.run_epoch(self.splitter.dev, e, 'VALID', grad = False)
				print("eval train, eval valid", eval_train, eval_valid)
				if eval_valid>best_eval_valid:
					best_embs = temp_node_embs
			...
			if len(self.splitter.test)>0 and eval_valid==best_eval_valid and e>self.args.eval_after_epochs:
				eval_test, _, pred_list, idx_list  = self.run_epoch(self.splitter.test, e, 'TEST', grad = False)
#print("pred list length",len(pred_list))
		...
		return best_eval_valid, best_embs, pred_list, idx_list
...
#	print(s.label_sp['idx'])
			loss = self.comp_loss(predictions,s.label_sp['vals'])
			# print(loss)
#				if set_name in ['TEST'] and self.args.task == 'link_pred':
#				test_pred.append(predictions)
			if set_name in ['TEST', 'VALID'] and self.args.task == 'link_pred':
				...
				pred_list.append(predictions) # add by kim
				idx_list.append(s.label_sp['idx'])
			if grad:
				self.optim_step(loss)

		torch.set_grad_enabled(True)
		eval_measure = self.logger.log_epoch_done()
#	print("epcoh pred_list length",len(pred_list))
		return eval_measure, nodes_embs, pred_list, idx_list # pred link
#return eval_measure, nodes_embs, predictions # pred edge
...
#			print("clsinputsize", cls_input.shape)
